    ...
    for (pp = proc; pp < &proc[NPROC]; pp++) {
      if (pp->parent == p) {
        ...
        if (pp->state == ZOMBIE) {
          ...
          if (w != 0 && copyout(p->pagetable, p->sz, w, (char *)&wt, sizeof(wt)) < 0) {
            release(&pp->lock);
            release(&wait_lock);
            return -1;
          }
...
scheduler(void)
{
  ...
  for (;;) {
    ...
    for (int q = 0; q < 4; q++) {
      ...
      for (p = proc; p < &proc[NPROC]; p++) {
        ...
        if (p->state == RUNNABLE && p->priority == q) {
          if (best == 0 || p->enter_time < best->enter_time) {
            if (best)
              release(&best->lock);
            best = p;
            continue;
    ...
    if (earliest != 0) {
      if (earliest->start_time == -1)
        earliest->start_time = ticks;
...
int
has_higher_priority_runnable(int current_priority)
{
  struct proc *p;
  for (p = proc; p < &proc[NPROC]; p++) {
    acquire(&p->lock);
...
procdump(void)
{
  ...
  printk("\nPID\tSTATE\tNAME\tPRIO\tSLICETK\tQ0\tQ1\tQ2\tQ3\tWAIT\tRUN\n");
  for (p = proc; p < &proc[NPROC]; p++) {
    if (p->state == UNUSED)
      continue;
    if (p->state >= 0 && p->state < NELEM(states) && states[p->state])
      ...
      state = "???";
    printk("%d\t%s\t%s\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",
           p->pid, state, p->name, p->priority, p->ticks_in_slice,
           p->q_ticks[0], p->q_ticks[1], p->q_ticks[2], p->q_ticks[3],
           p->wait_time, p->run_time);
  }
}
