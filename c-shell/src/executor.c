...
static int pipeline_is_empty(const Pipeline *p) {
    return p->stage_count == 1 &&
           p->stages[0].argc == 0 &&
           p->stages[0].input_count == 0 &&
           p->stages[0].output_count == 0;
}

static size_t consume_one_pipeline(const TokenList *tokens, size_t start, Pipeline *out, int *ended_with_amp) {
    out->stages = NULL;
    out->stage_count = 0;
...
void build_command_line(const TokenList *tokens, CommandLine *out) {
    ...
    while (pos < tokens->count) {
        Pipeline p;
        int ended_with_amp;
        pos = consume_one_pipeline(tokens, pos, &p, &ended_with_amp);
        ...
        out->segments = realloc(out->segments, sizeof(CommandSegment) * (size_t)(out->segment_count + 1));
        out->segments[out->segment_count].pipeline = p;
        out->segments[out->segment_count].background = ended_with_amp;
        out->segment_count++;
    }
}

void free_command_line(CommandLine *cl) {
    for (int i = 0; i < cl->segment_count; i++) {
        free_pipeline(&cl->segments[i].pipeline);
    }
    free(cl->segments);
    cl->segments = NULL;
    cl->segment_count = 0;
...
int launch_pipeline(Pipeline *pipeline, int background, pid_t *out_pgid, pid_t *out_pids) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (pid == 0) {
            ...
            if (background && i == 0 && s->prepared_stdin_fd < 0 && prev_read < 0) {
        ...
        if (i == 0) pgid = pid;
        setpgid(pid, pgid);
        if (pid_idx < MAX_PIPELINE_PIDS) {
            out_pids[pid_idx++] = pid;
        }
    }
    ...
    if (last_has_output) {
        ...
        if (tee_pid > 0) {
            setpgid(tee_pid, pgid);
            if (pid_idx < MAX_PIPELINE_PIDS) {
                out_pids[pid_idx++] = tee_pid;
            }
        }
    }
    ...
    for (int i = 0; i < n; i++) {
        Stage *s = &pipeline->stages[i];
        if (s->prepared_stdin_fd >= 0) {
            close(s->prepared_stdin_fd);
            s->prepared_stdin_fd = -1;
        }
        for (int j = 0; j < s->output_count; j++) {
            if (s->prepared_output_fds && s->prepared_output_fds[j] >= 0) {
                close(s->prepared_output_fds[j]);
                s->prepared_output_fds[j] = -1;
            }
        }
    }
...
void run_pipeline(Pipeline *pipeline) {
    pid_t pids[MAX_PIPELINE_PIDS];
    pid_t pgid;
    int count = launch_pipeline(pipeline, 0, &pgid, pids);
    if (count > 0) {
        wait_for_pipeline(pids, count);
    }
}
